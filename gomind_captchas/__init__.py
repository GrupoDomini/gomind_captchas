from src.web import wait_element, send_keys, click
...
def image_solver(driver, xpath_img=str, xpath_input=str, xpath_button=str):
    ...
    try:
        image_captcha = wait_element(driver, by=By.XPATH, element=xpath_img, time=20)
        ...
        if captcha_text != 0:
            ...
            send_keys(driver, by=By.XPATH, element=xpath_input, keys=captcha_text)
            sleep(0.5)
            click(driver, by=By.XPATH, element=xpath_button)
...
def recaptcha_solver(driver, xpath_submit, xpath_iframe, site_token=str, url=str, by=By.XPATH):
    ...
    url = url
    g_recaptcha_response = wait_element(driver, by=By.ID, element="g-recaptcha-response", time=10)
    ...
    driver.switch_to.frame(wait_element(driver, element=xpath_iframe, time=5))
    ...
    time.sleep(1)
    click(driver, time=5, by=by, element=xpath_submit)
